...
static inline vl_set_node* vl_SetGetNodeAt(vl_set* set, vl_set_iter iter){
    return iter == VL_SET_ITER_INVALID ? NULL : (vl_set_node*)vlLinearPoolSample(&set->nodePool, iter);
}
...
void vlSetInit(vl_set* set, vl_memsize_t elementSize, vl_compare_function compFunc){
    vlLinearPoolInit(&set->nodePool, sizeof(vl_set_node) + elementSize);
...
vl_transient* vlSetSample(vl_set* set, vl_set_iter iter){
    return vlLinearPoolSample(&set->nodePool, iter) + sizeof(vl_set_node);
}
...
vl_set_iter vlSetInsert(vl_set* set, const void* elem){
    ...
    if(curIter == VL_SET_ITER_INVALID){
        resultIter = vlLinearPoolTake(&set->nodePool);
        vl_set_node* newNode = (vl_set_node*)vlLinearPoolSample(&set->nodePool, resultIter);
    ...
    while(curIter != VL_SET_ITER_INVALID){
        vl_set_node* curNode = (vl_set_node*)vlLinearPoolSample(&set->nodePool, curIter);
        ...
        //Need to re-sample the current node pointer due to instantiation of
        //a new node, which could resize the underlying node pool
        //and thus invalidate the previous pointer.
        curNode = (vl_set_node*)vlLinearPoolSample(&set->nodePool, curIter);
        nextIter = comp > 0 ? &curNode->left : &curNode->right;
        ...
        if(nodeXIter == nodeXParent->left){
            ...
            if(leftColor == vl_rbtree_color_black && rightColor == vl_rbtree_color_black){
                ...
                if(rightColor == vl_rbtree_color_black){
                    nodeW->color = vl_rbtree_color_red;
                    vl_SetTreeRotateRight(set, vlLinearPoolTellIndex(&set->nodePool, (const vl_transient *) nodeW));
                    nodeXParent = vl_SetGetNodeAt(set, nodeX->parent);
                    nodeW = vl_SetGetNodeAt(set, nodeXParent->right);
                }
            ...
            if(leftColor == vl_rbtree_color_black && rightColor == vl_rbtree_color_black){
                ...
                if(leftColor == vl_rbtree_color_black){
                    nodeW->color = vl_rbtree_color_red;
                    vl_SetTreeRotateLeft(set, vlLinearPoolTellIndex(&set->nodePool, (const vl_transient *) nodeW));
                    nodeXParent = vl_SetGetNodeAt(set, nodeX->parent);
                    nodeW = vl_SetGetNodeAt(set, nodeXParent->left);
                }
...
void vlSetRemove(vl_set* set, vl_set_iter nodeZIter){
    ...
    vlLinearPoolReturn(&set->nodePool, nodeYIter);
}
...
void vlSetClear(vl_set* set){
    vlLinearPoolClear(&set->nodePool);
    set->root = VL_SET_ITER_INVALID;
}
...
vl_set* vlSetClone(const vl_set* src, vl_set* dest){
    ...
    vlLinearPoolClone(&src->nodePool, &dest->nodePool);
...
vl_set* vlSetUnion(vl_set* a, vl_set* b, vl_set* dest){
    ...
    VL_SET_FOREACH(a, curIter)
        vlSetInsert(dest, vlSetSample(a, curIter));
...
vl_set* vlSetIntersection(vl_set* a, vl_set* b, vl_set* dest){
    ...
    VL_SET_FOREACH(a, curIter){
        const vl_transient* const sample = vlSetSample(a, curIter);
