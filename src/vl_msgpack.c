...
vl_msgpack_iter
vlMsgPackInsert(vl_msgpack *pack, vl_msgpack_type type, vl_msgpack_iter parent, const void *keyPtr, vl_memsize_t keyLen,
                const void *dataPtr, vl_memsize_t dataLen);

vl_msgpack_element *
vl_MsgPackInitNode(vl_msgpack *pack, vl_msgpack_iter nodeIter, vl_msgpack_type type, vl_int8_t extType,
                   vl_msgpack_iter parent, const void *dataPtr, vl_memsize_t dataLen) {
    ...
    switch (type) {
        ...
        case VL_MSGPACK_ARRAY: {
            ...
            for (curIdx = 0; curIdx < length; curIdx++) {
                ...
                vl_msgpack_element *childElement = (vl_msgpack_element *) vlHashTableSampleValue(&pack->nodes,
                                                                                                 childIter, NULL);
...
void vlMsgPackInit(vl_msgpack *pack) {
    ...
    pack->root = vlMsgPackInsert(pack, VL_MSGPACK_MAP, VL_HASHTABLE_ITER_INVALID, ROOT_STRING, sizeof(ROOT_STRING),
                                 NULL, 0);
}
...
vl_msgpack_iter vlMsgPackInsertExt(vl_msgpack *pack, vl_msgpack_type type, vl_int8_t extType, vl_msgpack_iter parent,
    ...
    if (parent != VL_HASHTABLE_ITER_INVALID) {
        vl_msgpack_element *const parentElement = (vl_msgpack_element *) vlHashTableSampleValue(&pack->nodes, parent,
                                                                                                NULL);

        switch (parentElement->type) {
            case VL_MSGPACK_ARRAY: {
                const vl_dsidx_t ordinal = *(const vl_dsidx_t *) (keyPtr);
                vl_msgpack_iter *const array = (vl_msgpack_iter *) vlArenaMemSample(&pack->values,
                                                                                    parentElement->payload.arrayBranch.array);
            ...
            case VL_MSGPACK_MAP: {
                ...
                if (*oldTailIter != VL_MSGPACK_ITER_INVALID) {
                    ...
                    vl_msgpack_element *const siblingElement = (vl_msgpack_element *) vlHashTableSampleValue(
                            &pack->nodes, *oldTailIter, NULL);
