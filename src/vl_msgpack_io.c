...
vl_msgpack_iter vlMsgPackIODecodeToDOM(vl_msgpack_decoder* decoder, vl_msgpack* dest, vl_msgpack_iter parent, const void* key, vl_memsize_t keyLen){
    ...
    switch(curToken.type){
        case  VL_MSGPACK_ARRAY:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_ARRAY, parent, key, keyLen, &curToken.array.elements, sizeof(vl_dsidx_t));
            for(vl_dsidx_t curIndex = 0; curIndex < curToken.array.elements; curIndex++){
                if(vlMsgPackIODecodeToDOM(decoder, dest, childIter, &curIndex, sizeof(vl_dsidx_t)) == VL_MSGPACK_ITER_INVALID){
                    //Collapse on failure.
        ...
        case  VL_MSGPACK_MAP:
            ...
            for(vl_dsidx_t curPair = 0; curPair < curToken.map.keyValuePairs; curPair++){
                ...
                if(vlMsgPackIODecodeToDOM(decoder, dest, childIter, keyToken.string.ptr, keyToken.string.length) == VL_MSGPACK_ITER_INVALID){
                    //Collapse on failure.
                    vlMsgPackRemove(dest, childIter);
                    return VL_MSGPACK_ITER_INVALID;
                }
        ...
        case  VL_MSGPACK_NIL:
            ...
            break;
        case  VL_MSGPACK_BOOL:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_BOOL, parent, key, keyLen, &curToken.boolean.value, sizeof(vl_bool_t));
            break;
        case  VL_MSGPACK_INT:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_INT, parent, key, keyLen, &curToken.integer.value, sizeof(vl_ilarge_t));
            break;
        case  VL_MSGPACK_UINT:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_UINT, parent, key, keyLen, &curToken.uinteger.value, sizeof(vl_ularge_t));
            break;
        case  VL_MSGPACK_FLOAT32:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_FLOAT32, parent, key, keyLen, &curToken.float32.value, sizeof(vl_float32_t));
            break;
        case  VL_MSGPACK_FLOAT64:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_FLOAT64, parent, key, keyLen, &curToken.float64.value, sizeof(vl_float64_t));
            break;
        case  VL_MSGPACK_STRING:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_STRING, parent, key, keyLen, curToken.string.ptr, curToken.string.length);
            break;
        case  VL_MSGPACK_BINARY:
            childIter = vlMsgPackInsert(dest, VL_MSGPACK_BINARY, parent, key, keyLen, curToken.binary.ptr, curToken.binary.length);
            break;
        case  VL_MSGPACK_EXT:
            childIter = vlMsgPackInsertExt(dest, VL_MSGPACK_EXT, curToken.ext.extType, parent, key, keyLen, curToken.ext.ptr, curToken.ext.length);
            break;
    }
