 ...
 */
typedef struct{
    vl_fixedpool_idx prev;
    vl_fixedpool_idx next;
} vl_linked_list_node;

void vlListInit(vl_linked_list* list, vl_memsize_t elementSize){
    vlFixedPoolInit(&list->nodePool, elementSize + sizeof(vl_linked_list_node));
...
vl_list_iter vlListPushFront(vl_linked_list* list, const void* elem){
    ...
    vl_list_iter newHead = vlFixedPoolTake(&list->nodePool);

    void* dest = vlFixedPoolSample(&list->nodePool, newHead);
    ...
    if(oldHead != VL_LIST_ITER_INVALID){
        vl_linked_list_node* oldNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, oldHead);
...
void vlListPopFront(vl_linked_list* list){
    if(list->head == VL_LIST_ITER_INVALID)
        ...
        vlFixedPoolReturn(&list->nodePool, list->head);
        list->head = list->tail = VL_LIST_ITER_INVALID;
        return;
    }

    vl_linked_list_node* oldNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, list->head);
    const vl_list_iter rightIter = oldNode->next;

    vl_linked_list_node* rightNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, rightIter);
...
vl_list_iter vlListPushBack(vl_linked_list* list, const void* elem){
    ...
    const vl_list_iter newTail = vlFixedPoolTake(&list->nodePool);

    void* dest = vlFixedPoolSample(&list->nodePool, newTail);
    vl_linked_list_node* newNode = dest;
    dest = newNode + 1;
    ...
    if(oldTail != VL_LIST_ITER_INVALID){
        vl_linked_list_node* oldNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, oldTail);
...
void vlListPopBack(vl_linked_list* list){
    if(list->tail == VL_LIST_ITER_INVALID)
        ...
        vlFixedPoolReturn(&list->nodePool, list->tail);
        list->head = list->tail = VL_LIST_ITER_INVALID;
        return;
    }

    vl_linked_list_node* oldNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, list->tail);
    const vl_list_iter leftIter = oldNode->prev;

    vl_linked_list_node* leftNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, leftIter);
...
vl_list_iter vlListInsertAfter(vl_linked_list* list, vl_list_iter target, const void* elem){
    vl_linked_list_node* leftNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, target);
    if(leftNode->next == VL_LIST_ITER_INVALID)
        return vlListPushBack(list, elem);

    vl_linked_list_node* rightNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, leftNode->next);

    vl_list_iter newIter = vlFixedPoolTake(&list->nodePool);
    vl_linked_list_node* newNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, newIter);
...
vl_list_iter vlListInsertBefore(vl_linked_list* list, vl_list_iter target, const void* elem){
    vl_linked_list_node* rightNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, target);
    if(rightNode->prev == VL_LIST_ITER_INVALID)
        return vlListPushFront(list, elem);

    vl_linked_list_node* leftNode = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, rightNode->prev);
...
vl_linked_list* vlListClone(const vl_linked_list* src, vl_linked_list* dest){
    ...
    vlFixedPoolClone(&src->nodePool, &dest->nodePool);
    dest->head = src->head;
    dest->tail = src->tail;
...
vl_dsidx_t          vl_ListSortSegmentLength(vl_linked_list* list, vl_list_iter begin, vl_list_iter* stopIter){
    ...
    while(current != VL_LIST_ITER_INVALID){
        lastIter        = current;
        currentHeader   = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, current);
...
void                vl_ListSortMerge(vl_linked_list* list, vl_list_iter aIter, vl_list_iter bIter,
    ...
    vl_linked_list_node*    leftHeader     = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, aIter);
    vl_linked_list_node*    rightHeader    = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, bIter);
    vl_linked_list_node*    tailHeader;
    vl_transient* leftData  = (vl_transient*)(leftHeader + 1);
    vl_transient* rightData = (vl_transient*)(rightHeader + 1);
    ...
    *mergeTail = *mergeHead;
    ...
    while(aIter != VL_LIST_ITER_INVALID && bIter != VL_LIST_ITER_INVALID){
        leftHeader      = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, aIter);
        rightHeader     = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, bIter);
        tailHeader      = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, *mergeTail);
...
vl_list_iter        vl_ListSortSplit(vl_linked_list* list, vl_list_iter atIter, vl_dsidx_t length){
    ...
    for(int i = 0; i < length; i++){
        atHeader = (vl_linked_list_node*) vlFixedPoolSample(&list->nodePool, atIter);
    ...
    if(atHeader->next != VL_LIST_ITER_INVALID){
        ...
        splitHeader = (vl_linked_list_node*) vlFixedPoolSample(&list->nodePool, atHeader->next);
    ...
    while(passLength < length){
        ...
        while(currentIter != VL_LIST_ITER_INVALID){
            ...
            if(tailIter == VL_LIST_ITER_INVALID){
                ...
                vl_linked_list_node* const tailNode         = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, tailIter);
                vl_linked_list_node* const mergedHeadNode   = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, mergedHead);
...
vl_list_iter vlListFind(vl_linked_list* src, const void* element){
    VL_LIST_FOREACH(src, curIter)
        if(memcmp(vlListSample(src, curIter), element, src->elementSize) == 0)
            return curIter;
    return VL_LIST_ITER_INVALID;
}

void        vlListRemove(vl_linked_list* list, vl_list_iter iter){
    vl_linked_list_node* oldNode    = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, iter);
    ...
    vlFixedPoolReturn(&list->nodePool, iter);
}

vl_list_iter vlListNext(vl_linked_list* list, vl_list_iter iter){
    vl_linked_list_node* const node = (vl_linked_list_node*)vlFixedPoolSample(&list->nodePool, iter);
    return node->next;
}
