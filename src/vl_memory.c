...
vl_memory *vlMemRealloc(vl_memory *mem, vl_memsize_t allocSize) {
    ...
    vl_memory *const origin = realloc(VL_MEMORY_ORIGIN_INLINE(VL_MEMORY_HEADER_INLINE(mem)),
                                      newSize + oldHeader.alignment);
...
vl_int_t
vl_MemSortPartition(vl_usmall_t *buffer, void *pivot, void *swap, vl_int_t low, vl_int_t high, vl_memsize_t elementSize,
...
void
vlMemCopyStride(const void *src, vl_dsoffs_t srcStride, void *dest, vl_dsoffs_t dstStride, vl_memsize_t elementSize,
