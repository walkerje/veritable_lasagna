...
vl_stack_offset vlStackPush(vl_stack *stack, vl_memsize_t size) {
    const vl_memsize_t alignedSize = VL_MEMORY_PAD_UP(size + sizeof(vl_stack_header), VL_DEFAULT_MEMORY_ALIGN);
    const vl_uintptr_t head = vlBufferWrite(&stack->buffer, alignedSize, NULL);
...
vl_memsize_t vlStackSampleSize(vl_stack *stack, vl_stack_offset offset) {
    vl_stack_header *header = (vl_stack_header *) ((vl_usmall_t *) (stack->buffer.data) + offset -
                                                   sizeof(vl_stack_header));
    return header->size;
}
