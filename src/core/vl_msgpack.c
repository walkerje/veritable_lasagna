...
/**
 * \brief Maximum hierarchical key size to allocate on stack.
 *
 * For msgpack, a typical key is: [parent_hash (8) + user_key (var)]
 * Most JSON-like keys are < 256 bytes, so 512 provides safe margin.
 * If keys are larger, consider increasing (stack space permits).
 * If stack-constrained (embedded), decrease to 256.
 *
...
/**
 * \brief Helper for managing hierarchical keys (stack vs arena).
 * Uses stack allocation for small keys, falls back to arena for large ones.
 * \private
 */
typedef struct vl_msgpack_key_buffer_
{
    vl_uint8_t* keyData; // Points to either stackBuffer or arena allocation
    vl_memsize_t keySize; // Total key size
    vl_arena_ptr arenaAlloc; // Non-zero if allocated from arena
} vl_msgpack_key_buffer;
...
typedef struct vl_msgpack_element_
{
    ...
    union
    {
        ...
        struct
        {
            ...
            // Maps are initialized as empty (firstChild = VL_MSGPACK_ITER_INVALID;
            // totalChildren = 0)
...
/**
 * \brief Initialize and construct a hierarchical key.
 * Tries stack allocation first, falls back to arena if needed.
 * \private
 * \param pack The msgpack instance
 * \param parentHash Hash of the parent node
 * \param keyPtr Pointer to user-provided key data
 * \param keyLen Length of user-provided key
 * \param stackBuffer Pre-allocated stack buffer
 * \param stackSize Size of stack buffer
 * \return Initialized key buffer structure
 */
static vl_msgpack_key_buffer vl_MsgPackKeyBegin(vl_msgpack* pack, vl_hash parentHash, const void* keyPtr,
                                                vl_memsize_t keyLen, vl_uint8_t* stackBuffer, vl_memsize_t stackSize)
{
    vl_msgpack_key_buffer keyBuffer;
    const vl_memsize_t totalKeySize = sizeof(vl_hash) + keyLen;

    // Try stack allocation first
    if (totalKeySize <= stackSize)
    {
        keyBuffer.keyData = stackBuffer;
        keyBuffer.arenaAlloc = VL_ARENA_NULL;
    }
    else
    {
        // Fall back to arena allocation
        keyBuffer.arenaAlloc = vlArenaMemAlloc(&pack->values, totalKeySize);
        keyBuffer.keyData = (vl_uint8_t*)vlArenaMemSample(&pack->values, keyBuffer.arenaAlloc);
    }

    // Fill the key: [parentHash][keyData]
    memcpy(keyBuffer.keyData, &parentHash, sizeof(vl_hash));
    memcpy(keyBuffer.keyData + sizeof(vl_hash), keyPtr, keyLen);
    keyBuffer.keySize = totalKeySize;

    return keyBuffer;
}

/**
 * \brief Clean up a hierarchical key buffer.
 * Frees arena allocation if one was made, otherwise does nothing.
 * \private
 * \param pack The msgpack instance
 * \param keyBuffer The key buffer to clean up
 */
static inline void vl_MsgPackKeyEnd(vl_msgpack* pack, vl_msgpack_key_buffer* keyBuffer)
{
    if (keyBuffer->arenaAlloc != VL_ARENA_NULL)
    {
        vlArenaMemFree(&pack->values, keyBuffer->arenaAlloc);
        keyBuffer->arenaAlloc = VL_ARENA_NULL;
    }
}
...
// This is a recursive function that descends the tree relative to the given
// iterator.
void vl_MsgPackFreeChildren(vl_msgpack* pack, vl_msgpack_iter iter)
{
    ...
    case VL_MSGPACK_MAP:
        {
            ...
            while (curIter != VL_MSGPACK_ITER_INVALID)
            {
                ...
                vlHashTableRemoveIter(&pack->nodes,
                                      curIter); // Remove child node from the tree
    ...
    case VL_MSGPACK_NIL:
        break;
    default:
        // Reached a terminal. Simply free its data and remove it from the node
        // table.
...
vl_msgpack_iter vlMsgPackInsert(vl_msgpack* pack, vl_msgpack_type type, vl_msgpack_iter parent, const void* keyPtr,
                                vl_memsize_t keyLen, const void* dataPtr, vl_memsize_t dataLen);

vl_msgpack_element* vl_MsgPackInitNode(vl_msgpack* pack, vl_msgpack_iter nodeIter, vl_msgpack_type type,
                                       vl_int8_t extType, vl_msgpack_iter parent, const void* dataPtr,
    ...
    case VL_MSGPACK_ARRAY:
        {
            ...
            for (curIdx = 0; curIdx < length; curIdx++)
            {
                ...
                vl_msgpack_element* childElement =
                    (vl_msgpack_element*)vlHashTableSampleValue(&pack->nodes, childIter, NULL);
            ...
            // Re-obtain a pointer to the array node; might have moved due to NIL
            // initialization.
...
void vlMsgPackInit(vl_msgpack* pack)
{
    ...
    pack->root =
        vlMsgPackInsert(pack, VL_MSGPACK_MAP, VL_HASHTABLE_ITER_INVALID, ROOT_STRING, sizeof(ROOT_STRING), NULL, 0);
}
...
vl_msgpack_iter vlMsgPackInsertExt(vl_msgpack* pack, vl_msgpack_type type, vl_int8_t extType, vl_msgpack_iter parent,
                                   const void* keyPtr, vl_memsize_t keyLen, const void* dataPtr, vl_memsize_t dataLen)
{
    // Immediately discard attempts to insert an EXT type with no subtype.
    if (type == VL_MSGPACK_EXT && extType == VL_MSGPACK_EXT_NONE)
        return VL_MSGPACK_ITER_INVALID;

    // Allocate stack buffer for typical key sizes
    vl_uint8_t stackKeyBuffer[VL_MSGPACK_MAX_STACK_KEY_SIZE];

    // Get parent's hash (or use a default for root)
    vl_hash parentHash;
    if (parent != VL_MSGPACK_ITER_INVALID)
    {
        ...
        parentHash = parentHeader->keyHash;
    }
    else
    {
        // Root node insertion: use FNV1A prime mixed with an offset.
        parentHash = vlHashCombine(0x00000100000001b3, 0xcbf29ce484222325);
    }

    // Construct hierarchical key (stack-based when possible)
    vl_msgpack_key_buffer keyBuffer =
        vl_MsgPackKeyBegin(pack, parentHash, keyPtr, keyLen, stackKeyBuffer, sizeof(stackKeyBuffer));

    vl_msgpack_iter nodeIter = vlHashTableFind(&pack->nodes, keyBuffer.keyData, keyBuffer.keySize);

    if (nodeIter != VL_MSGPACK_ITER_INVALID)
    {
        // Node already exists in this slot. Explicitly overwrite it.

        // Clean up the key buffer (only frees if arena was used)
        vl_MsgPackKeyEnd(pack, &keyBuffer);

        // Then see if it has an existing value for its leaf arena field, or any
        // existing children.
    ...
    nodeIter = vlHashTableInsert(&pack->nodes, keyBuffer.keyData, keyBuffer.keySize, sizeof(vl_msgpack_element));
    ...
    {
        vl_msgpack_element* const parentElement =
            (vl_msgpack_element*)vlHashTableSampleValue(&pack->nodes, parent, NULL);
        ...
        case VL_MSGPACK_ARRAY:
            {
                ...
                vl_msgpack_iter* const array =
                    (vl_msgpack_iter*)vlArenaMemSample(&pack->values, parentElement->payload.arrayBranch.array);
        ...
        case VL_MSGPACK_MAP:
            {
                ...
                if (*oldTailIter != VL_MSGPACK_ITER_INVALID)
                {
                    ...
                    vl_msgpack_element* const siblingElement =
                        (vl_msgpack_element*)vlHashTableSampleValue(&pack->nodes, *oldTailIter, NULL);
    ...
    case VL_MSGPACK_MAP:
        {
            ...
            {
                vl_msgpack_element* rightElem = (vl_msgpack_element*)vlHashTableSampleValue(&pack->nodes, right, NULL);
...
vl_msgpack_iter vlMsgPackFindChild(vl_msgpack* pack, vl_msgpack_iter parent, const void* keyPtr, vl_memsize_t keyLen)
{
    ...
    // Get parent's hash from the hashtable header
    vl_hashtable_header const* parentHeader = (vl_hashtable_header*)vlArenaMemSample(&pack->nodes.data, parent);

    // Construct hierarchical key (stack-based when possible)
    vl_msgpack_key_buffer keyBuffer =
        vl_MsgPackKeyBegin(pack, parentHeader->keyHash, keyPtr, keyLen, stackKeyBuffer, sizeof(stackKeyBuffer));

    // Perform the lookup
    const vl_msgpack_iter nodeIter = vlHashTableFind(&pack->nodes, keyBuffer.keyData, keyBuffer.keySize);

    // Clean up (only frees if arena was used)
    vl_MsgPackKeyEnd(pack, &keyBuffer);
...
{
    // Indexed keys are a simple tuple of the parent hash and the resulting
    // ordinal. We can construct a key for it without touching any dynamic
    // allocation by constructing a key in a buffer of a well-known size.
