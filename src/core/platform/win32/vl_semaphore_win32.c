...
{
    HANDLE sem = CreateSemaphore(NULL, // Default security attributes
...
void vlSemaphoreDelete(vl_semaphore sem) { CloseHandle((HANDLE)sem); }

vl_bool_t vlSemaphoreWait(vl_semaphore sem, vl_uint_t timeoutMs)
{
    DWORD result = WaitForSingleObject((HANDLE)sem, timeoutMs == 0 ? INFINITE : timeoutMs);

    return result == WAIT_OBJECT_0;
}

void vlSemaphorePost(vl_semaphore sem) { ReleaseSemaphore((HANDLE)sem, 1, NULL); }
