...
static void vl_SocketSetErrorFromErrno(void) { vl_SocketSetError(strerror(errno)); }

vl_socket_result vlSocketStartup(void) { return VL_SOCKET_SUCCESS; }
...
vl_socket vlSocketAccept(vl_socket socket, vl_socket_address* outAddress)
{
    ...
    int client_fd = accept(socket->fd, (struct sockaddr*)&addr, &addr_len);

    if (client_fd < 0)
    {
        if (errno == EAGAIN || errno == EWOULDBLOCK)
        {
            // Not necessarily an error we want to log text for every time in non-blocking
        }
        else
        {
            vl_SocketSetErrorFromErrno();
...
vl_socket_result vlSocketConnect(vl_socket socket, const vl_socket_address* address)
{
    ...
    if (connect(socket->fd, (struct sockaddr*)&addr, addr_len) < 0)
    {
...
vl_int64_t vlSocketSend(vl_socket socket, const void* buffer, vl_memsize_t length)
{
    ...
    ssize_t sent = send(socket->fd, buffer, length, 0);
    if (sent < 0)
    {
        vl_SocketSetErrorFromErrno();
        return -1;
...
vl_int64_t vlSocketReceive(vl_socket socket, void* buffer, vl_memsize_t length)
{
    ...
    ssize_t received = recv(socket->fd, buffer, length, 0);
    if (received < 0)
    {
        if (errno == EAGAIN || errno == EWOULDBLOCK)
        {
            return -1; // Should we have a way to distinguish? API says -1 on error.
        }
        vl_SocketSetErrorFromErrno();
        return -1;
    }
...
vl_socket_result vlSocketSetNoDelay(vl_socket socket, vl_bool_t enabled)
{
    ...
    int opt = enabled ? 1 : 0;
    if (setsockopt(socket->fd, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) < 0)
    {
        vl_SocketSetErrorFromErrno();
        return VL_SOCKET_ERROR_SET_OPTION;
    }
...
vl_socket_result vlSocketGetReuseAddress(vl_socket socket, vl_bool_t* outEnabled)
{
    ...
    if (getsockopt(socket->fd, SOL_SOCKET, SO_REUSEADDR, &opt, &len) < 0)
    {
        vl_SocketSetErrorFromErrno();
        return VL_SOCKET_ERROR_SET_OPTION;
    }
...
vl_socket_result vlSocketGetNoDelay(vl_socket socket, vl_bool_t* outEnabled)
{
    ...
    int opt = 0;
    socklen_t len = sizeof(opt);
    if (getsockopt(socket->fd, IPPROTO_TCP, TCP_NODELAY, &opt, &len) < 0)
    {
        vl_SocketSetErrorFromErrno();
        return VL_SOCKET_ERROR_SET_OPTION;
    }
...
vl_socket_result vlSocketGetKeepAlive(vl_socket socket, vl_bool_t* outEnabled)
{
    ...
    int opt = 0;
    socklen_t len = sizeof(opt);
    if (getsockopt(socket->fd, SOL_SOCKET, SO_KEEPALIVE, &opt, &len) < 0)
    {
        vl_SocketSetErrorFromErrno();
        return VL_SOCKET_ERROR_SET_OPTION;
    }
...
vl_bool_t vlSocketAddressToString(const vl_socket_address* address, char* buffer, vl_memsize_t bufferSize)
{
    ...
    if (address->domain == VL_SOCKET_DOMAIN_IPV4)
    {
        ...
        int written = snprintf(buffer, bufferSize, "%s:%u", addrStr, address->port);
...
vl_socket_result vlSocketGetLocalAddress(vl_socket socket, vl_socket_address* outAddress)
{
    ...
    struct sockaddr_storage addr;
    socklen_t addr_len = sizeof(addr);
    if (getsockname(socket->fd, (struct sockaddr*)&addr, &addr_len) < 0)
    {
        vl_SocketSetErrorFromErrno();
        return VL_SOCKET_ERROR_BIND; // Reusing BIND error as it's address related
    }
