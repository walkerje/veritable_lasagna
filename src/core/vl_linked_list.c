...
vl_list_iter vlListPushFront(vl_linked_list* list, const void* elem)
{
    ...
    {
        vl_linked_list_node* oldNode = vlPoolSample(&list->nodePool, oldHead);
...
void vlListPopFront(vl_linked_list* list)
{
    ...
    vl_linked_list_node* oldNode = vlPoolSample(&list->nodePool, list->head);
    const vl_list_iter rightIter = oldNode->next;

    vl_linked_list_node* rightNode = vlPoolSample(&list->nodePool, rightIter);
...
vl_list_iter vlListPushBack(vl_linked_list* list, const void* elem)
{
    ...
    {
        vl_linked_list_node* oldNode = vlPoolSample(&list->nodePool, oldTail);
...
void vlListPopBack(vl_linked_list* list)
{
    ...
    vl_linked_list_node* oldNode = vlPoolSample(&list->nodePool, list->tail);
    const vl_list_iter leftIter = oldNode->prev;

    vl_linked_list_node* leftNode = vlPoolSample(&list->nodePool, leftIter);
...
{
    vl_linked_list_node* leftNode = vlPoolSample(&list->nodePool, target);
    if (leftNode->next == VL_LIST_ITER_INVALID)
        return vlListPushBack(list, elem);

    vl_linked_list_node* rightNode = vlPoolSample(&list->nodePool, leftNode->next);

    vl_list_iter newIter = vlPoolTake(&list->nodePool);
    vl_linked_list_node* newNode = vlPoolSample(&list->nodePool, newIter);
...
{
    vl_linked_list_node* rightNode = vlPoolSample(&list->nodePool, target);
    if (rightNode->prev == VL_LIST_ITER_INVALID)
        return vlListPushFront(list, elem);

    vl_linked_list_node* leftNode = vlPoolSample(&list->nodePool, rightNode->prev);
...
/**
 * \brief Iteratively traverses a linked list to determine its size and last
 * node.
 * \param list pointer
...
vl_dsidx_t vl_ListSortSegmentLength(vl_linked_list* list, vl_list_iter begin, vl_list_iter* stopIter)
{
    ...
    {
        lastIter = current;
        currentHeader = vlPoolSample(&list->nodePool, current);
 ...
 * \private
 */
void vl_ListSortMerge(vl_linked_list* list, vl_list_iter aIter, vl_list_iter bIter, vl_list_iter* mergeHead,
                      vl_list_iter* mergeTail, vl_compare_function compareFunction)
{
    ...
    vl_linked_list_node* leftHeader = vlPoolSample(&list->nodePool, aIter);
    vl_linked_list_node* rightHeader = vlPoolSample(&list->nodePool, bIter);
    ...
    *mergeTail = *mergeHead;
    ...
    {
        leftHeader = vlPoolSample(&list->nodePool, aIter);
        rightHeader = vlPoolSample(&list->nodePool, bIter);
        tailHeader = vlPoolSample(&list->nodePool, *mergeTail);
...
/**
 * \brief Splits the specified list into two parts, with the first part having
 ...
 * \private
 * \return iterator to where the split occurred (first iterator in the second
 * half of the split)
 */
vl_list_iter vl_ListSortSplit(vl_linked_list* list, vl_list_iter atIter, vl_dsidx_t length)
{
    if (atIter == VL_LIST_ITER_INVALID || length <= 0)
        return VL_LIST_ITER_INVALID;
    vl_linked_list_node *atHeader = NULL, *splitHeader;
    vl_list_iter splitIter = VL_LIST_ITER_INVALID;

    for (vl_dsidx_t i = 0; i < length; i++)
    {
        atHeader = vlPoolSample(&list->nodePool, atIter);
    ...
    if (atHeader->next != VL_LIST_ITER_INVALID)
    {
        ...
        splitHeader = vlPoolSample(&list->nodePool, atHeader->next);
...
void vlListSort(vl_linked_list* list, vl_compare_function compareFunction)
{
    ...
    while (passLength < length)
    {
        ...
        while (currentIter != VL_LIST_ITER_INVALID)
        {
            ...
            {
                vl_linked_list_node* const tailNode = vlPoolSample(&list->nodePool, tailIter);
                vl_linked_list_node* const mergedHeadNode = vlPoolSample(&list->nodePool, mergedHead);
...
{
    vl_linked_list_node* oldNode = vlPoolSample(&list->nodePool, iter);
...
{
    vl_linked_list_node* const node = vlPoolSample(&list->nodePool, iter);
    return node->next;
}
