...
typedef struct vl_memory_header
{
    ...
    vl_uint_t alignment; // byte alignment of user-end pointer. For unaligned
                         // allocations, this defaults to VL_DEFAULT_MEMORY_ALIGN.
} vl_memory_header;
...
vl_memory* vlMemAlloc(vl_memsize_t allocSize)
{
    ...
    return (vl_memory*)((vl_uint8_t*)header + sizeof(vl_memory_header));
}

vl_memory* vlMemAllocAligned(vl_memsize_t size, vl_uint_t align)
{
    if (align <= VL_DEFAULT_MEMORY_ALIGN)
        return vlMemAlloc(size); // lowest that can be specified is the largest
                                 // possible word size
...
{
    const vl_uint16_t align = (vl_uint16_t)(mem ? VL_MEMORY_HEADER_INLINE(mem)->alignment : VL_DEFAULT_MEMORY_ALIGN);

    if (mem == NULL)
        return vlMemAlloc(allocSize);

    if (align <= VL_DEFAULT_MEMORY_ALIGN)
    {
        void* origin = (vl_uint8_t*)mem - sizeof(vl_memory_header);
    ...
    vl_memory* const origin =
        realloc(VL_MEMORY_ORIGIN_INLINE(VL_MEMORY_HEADER_INLINE(mem)), newSize + oldHeader.alignment);
...
vl_memory* vlMemClone(vl_memory* mem)
{
    ...
    const vl_uint_t align = VL_MEMORY_HEADER_INLINE(mem)->alignment;
    vl_memory* alloc;

    if (align <= VL_DEFAULT_MEMORY_ALIGN)
        alloc = vlMemAlloc(size);
    else
        alloc = vlMemAllocAligned(size, align);
...
vl_int_t vl_MemSortPartition(vl_usmall_t* buffer, void* pivot, void* swap, vl_int_t low, vl_int_t high,
                             vl_memsize_t elementSize, vl_compare_function comparator)
{
...
void vl_MemSortQuicksort(vl_int_t* stack, void* buffer, void* pivot, void* swap, vl_memsize_t elementSize,
...
void vlMemSort(void* buffer, vl_memsize_t elementSize, vl_dsidx_t numElements, vl_compare_function comparator)
{
    ...
    vl_usmall_t* const tempMem = vlMemAlloc(sortIndexSize + (elementSize * 2));
    if (tempMem == NULL)
        return;
...
void vlMemCopyStride(const void* src, vl_dsoffs_t srcStride, void* dest, vl_dsoffs_t dstStride,
                     vl_memsize_t elementSize, vl_dsidx_t numElements)
{
    ...
    for (vl_dsidx_t i = 0; i < numElements; i++)
    {
...
{
    const vl_dsidx_t numSteps = (vl_dsidx_t)(elementSize / 2);
...
{
    if (!mem)
        return VL_DEFAULT_MEMORY_ALIGN;
