...
#else
/**
 * \brief Equivalent to a generic htnohl/htons/ntohl/ntohs, which convert
 * to/from system to network byte order.
 *
...
vl_uint8_t vl_MsgPackEncodeFrame(vl_msgpack_encframe* frame, vl_dsoffs_t* offset)
{
    if (!frame->isArray)
    {
        // Is a map? Map lengths are defined as total number of key-value pairs, so
        // divide by 2.
...
vl_bool_t vl_MsgPackEncodePop(vl_msgpack_encoder* enc)
{
    ...
    if (offset > 0)
    {
        ...
        enc->buffer.offset -= offset; // Offset the write pointer downwards the
...
vl_msgpack_encoder* vlMsgPackIOEncoderNew(void)
{
...
void vlMsgPackIOEncoderClear(vl_msgpack_encoder* enc)
{
    ...
    vlBufferReset(&enc->buffer, vlMemSize(enc->buffer.data));
    enc->error = VL_MSGPACK_IO_ERR_NONE;
    enc->errorType = VL_MSGPACK_NIL;
    enc->errorDepth = 0;
}
...
void vlMsgPackIOEncodeStringLen(vl_msgpack_encoder* enc, const char* value, vl_uint32_t len)
{
    ...
    {
        // fixstring
        const vl_uint8_t fixLen = (vl_uint8_t)(len) & 0x1F;
...
void vlMsgPackIOEncodeExt(vl_msgpack_encoder* enc, vl_int8_t type, const void* value, vl_uint32_t len)
{
    ...
    if (isFixed)
    { // Fixed EXT types must have a size <= 16.
        vl_int8_t encLen = 0;
        vl_uint8_t encType = 0;
...
void vlMsgPackIOEncodeInt(vl_msgpack_encoder* enc, vl_ilarge_t value)
{
    ...
    else if (value <= VL_MSGPACK_NEG_FIXINT_MAX && value >= VL_MSGPACK_NEG_FIXINT_MIN)
    {
...
void vlMsgPackIOEncodeFromDOM(vl_msgpack_encoder* enc, vl_msgpack* src, vl_msgpack_iter curIter)
{
    ...
    case VL_MSGPACK_MAP:
        ...
        VL_MSGPACK_FOREACH_CHILD(src, curIter, childIter)
        {
            ...
            vlMsgPackIOEncodeStringLen(enc, keyStr, (vl_uint32_t)keyLen);
            // RECURSIVE CALL HERE
            vlMsgPackIOEncodeFromDOM(enc, src, childIter);
        }
    ...
    case VL_MSGPACK_STRING:
        {
            ...
            vlMsgPackIOEncodeStringLen(enc, (const char*)strValue, (vl_uint32_t)strSize);
            break;
        }
    case VL_MSGPACK_BINARY:
        {
            ...
            vlMsgPackIOEncodeBinary(enc, binValue, (vl_uint32_t)binSize);
            break;
        }
    case VL_MSGPACK_EXT:
        {
            ...
            vlMsgPackIOEncodeExt(enc, extType, extValue, (vl_uint32_t)extSize);
            break;
        }
    };
}
...
vl_bool_t vlMsgPackIODecoderNext(vl_msgpack_decoder* dec, vl_msgpack_io_token* tkPtr)
{
    ...
    case VL_MSGPACK_IO_FLOAT32:
        {
            ...
            memcpy(&tkPtr->float32.value, dec->srcMem + dec->srcOffset, sizeof(vl_float32_t));
    ...
    default:
        ...
        if (typeTag <= 0x7F)
        {
            // Positive Fixint
            tkPtr->type = VL_MSGPACK_INT;
            tkPtr->integer.value = +(vl_ilarge_t)(typeTag & 0x7F);
        }
        else if (typeTag >= 0xE0)
        {
            tkPtr->type = VL_MSGPACK_INT;
            tkPtr->integer.value = (vl_int8_t)typeTag; // Sign extends naturally
        }
        else if (typeTag >= 0x80 && typeTag <= 0x8F)
        {
            // Fixmap
            tkPtr->type = VL_MSGPACK_MAP;
            tkPtr->map.keyValuePairs = (vl_dsidx_t)(typeTag & 0xF);
        }
        else if (typeTag >= 0x90 && typeTag <= 0x9F)
        {
            // Fixarray
            tkPtr->type = VL_MSGPACK_ARRAY;
            tkPtr->array.elements = (vl_dsidx_t)(typeTag & 0xF);
        }
        else if (typeTag >= 0xA0 && typeTag <= 0xBF)
        {
            // Fixstr
            tkPtr->type = VL_MSGPACK_STRING;
            tkPtr->string.length = (vl_uint32_t)(typeTag & 0x1F);
            _VL_DECODE_EOFCHECK(dec, tkPtr->string.length);
            tkPtr->string.ptr = (const char*)(dec->srcMem + dec->srcOffset);
            dec->srcOffset += tkPtr->string.length;
        }
...
vl_msgpack_iter vlMsgPackIODecodeToDOM(vl_msgpack_decoder* decoder, vl_msgpack* dest, vl_msgpack_iter parent,
                                       const void* key, vl_memsize_t keyLen)
{
    vl_msgpack_io_token curToken;
    vl_msgpack_iter childIter = VL_MSGPACK_ITER_INVALID;
    ...
    case VL_MSGPACK_ARRAY:
        childIter =
            vlMsgPackInsert(dest, VL_MSGPACK_ARRAY, parent, key, keyLen, &curToken.array.elements, sizeof(vl_dsidx_t));
    ...
    case VL_MSGPACK_BOOL:
        childIter =
            vlMsgPackInsert(dest, VL_MSGPACK_BOOL, parent, key, keyLen, &curToken.boolean.value, sizeof(vl_bool_t));
        break;
    case VL_MSGPACK_INT:
        childIter =
            vlMsgPackInsert(dest, VL_MSGPACK_INT, parent, key, keyLen, &curToken.integer.value, sizeof(vl_ilarge_t));
        break;
    case VL_MSGPACK_UINT:
        childIter =
            vlMsgPackInsert(dest, VL_MSGPACK_UINT, parent, key, keyLen, &curToken.uinteger.value, sizeof(vl_ularge_t));
        break;
    case VL_MSGPACK_FLOAT32:
    ...
    case VL_MSGPACK_STRING:
        childIter =
            vlMsgPackInsert(dest, VL_MSGPACK_STRING, parent, key, keyLen, curToken.string.ptr, curToken.string.length);
        break;
    case VL_MSGPACK_BINARY:
        childIter =
            vlMsgPackInsert(dest, VL_MSGPACK_BINARY, parent, key, keyLen, curToken.binary.ptr, curToken.binary.length);
        break;
    case VL_MSGPACK_EXT:
