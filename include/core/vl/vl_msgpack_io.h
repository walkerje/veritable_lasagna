...
typedef enum vl_msgpack_io_error_
{
    ...
    VL_MSGPACK_IO_ERR_STACK_UNDERFLOW, // Erroneous call to a container end
                                       // encoding function.
    VL_MSGPACK_IO_ERR_UNEXPECTED_TOKEN, // Expected a different token as "next"
                                        // when decoding.
    VL_MSGPACK_IO_ERR_UNBALANCED_MAP, // Map does not have an even number of
                                      // values when encoding (must be key/value
                                      // pairs)
    VL_MSGPACK_IO_ERR_UNEXPECTED_EOF, // Found end of buffer at an unexpected
                                      // point when decoding
} vl_msgpack_io_error;
 ...
 *
 * This structure holds state for the encoding of information in the MessagePack
 * format. The encoder is completely compliant with the MessagePack spec.
 *
 ...
 *
 * This encoder attempts to generates the smallest possible (smallest size)
 * result in all cases.
 */
 ...
 *
 * This function initializes the given MessagePack encoder, preparing it for
 * further encoding operations. It must be called before encoding data into
 * MessagePack format.
 *
 ...
 *
 * This function frees the resources allocated for the given MessagePack
 * encoder. It must be called after the encoder has been initialized with \ref
 * vlMsgPackIOEncoderInit. This function does not deallocate the encoder
 * structure.
 *
 ...
 *
 * This function allocates memory for a new MessagePack encoder and initializes
 * it. The returned pointer must be freed using \ref vlMsgPackIOEncoderDelete.
 *
 ...
 *
 * This function deletes the given MessagePack encoder, freeing all associated
 * resources. It must be called after the encoder has been created with \ref
 * vlMsgPackIOEncoderNew. The function deallocates the entire encoder structure.
 *
 ...
 *
 * This function serializes the beginning of a map (key-value pair container)
 * into the MessagePack format. It should be followed by calls to encode
 * individual keys and values before the map is closed with \ref
 ...
 *
 * This function serializes the end of a map (key-value pair container) into the
 * MessagePack format. It follows \ref vlMsgPackIOEncodeMapBegin and should be
 * called once all key-value pairs for the map have been encoded.
 *
 ...
 *
 * This function serializes the beginning of an array (list of values) into the
 * MessagePack format. It should be followed by calls to encode individual
 * values before the array is closed with \ref vlMsgPackIOEncodeArrayEnd.
 *
 * \param enc Pointer to the encoder context.
 */
 ...
 *
 * This function serializes the end of an array (list of values) into the
 * MessagePack format. It follows \ref vlMsgPackIOEncodeArrayBegin and should be
 * called once all elements of the array have been encoded.
 *
 ...
 *
 * This function serializes a boolean value (true or false) into the MessagePack
 * format and appends it to the encoder's buffer.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes the length of a string value into the MessagePack
 * format. It is often used in combination with the string value itself for
 * encoding large strings efficiently.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes a string value into the MessagePack format and
 * appends it to the encoder's buffer.
 *
 * \param enc Pointer to the encoder context.
...
{
    vlMsgPackIOEncodeStringLen(enc, value, (vl_uint32_t)strlen(value));
}

/**
 * \brief Encodes a 32-bit floating point value into the MessagePack stream.
 *
 * This function serializes a 32-bit floating point value into the MessagePack
 * format and appends it to the encoder's buffer.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes a 64-bit floating point value into the MessagePack
 * format and appends it to the encoder's buffer.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes a binary data value into the MessagePack format and
 * appends it to the encoder's buffer. The length of the binary data must be
 * provided.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes an extension type (custom format) into the
 * MessagePack format and appends it to the encoder's buffer. The extension type
 * identifier and the associated value are serialized.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes a signed integer value into the MessagePack format
 * and appends it to the encoder's buffer.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes an unsigned integer value into the MessagePack
 * format and appends it to the encoder's buffer.
 *
 * \param enc Pointer to the encoder context.
 ...
 *
 * This function serializes a `nil` value (similar to `null`) into the
 * MessagePack format and appends it to the encoder's buffer.
 *
 * \param enc Pointer to the encoder context.
 */
 ...
 *
 * Represents a single decoded value from a stream of encoded MessagePack
 * values.
 */
 ...
 *
 * The only job of the decoder is to produce a stream of decoded tokens from an
 ...
 *
 * This function initializes the MessagePack decoder with the given source
 * memory and length. It prepares the decoder for further operations, such as
 * token extraction or decoding to a DOM.
 *
 * \param dec Pointer to the MessagePack decoder to initialize.
 * \param srcMem Pointer to the source memory containing the MessagePack data to
 * decode.
 ...
 *
 * This function decodes the next token from the MessagePack data. It returns \c
 * true if a token was successfully extracted, or \c false if there are no more
 * tokens or the decoding process encounters an error.
 *
 * \param decoder Pointer to the MessagePack decoder.
 * \param tkPtr Pointer to the token structure where the extracted token will be
 * stored.
 ...
 *
 * This function decodes MessagePack data and inserts it into the specified DOM
 * at the given parent node. The decoded data is inserted using the provided
 * key.
 *
 * \param decoder Pointer to the MessagePack decoder.
 * \param dest Pointer to the MessagePack DOM to populate.
 * \param parent The parent node in the DOM where the decoded data will be
 * inserted.
 ...
 */
VL_API vl_msgpack_iter vlMsgPackIODecodeToDOM(vl_msgpack_decoder* decoder, vl_msgpack* dest, vl_msgpack_iter parent,
                                              const void* key, vl_memsize_t keyLen);
