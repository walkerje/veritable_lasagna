...
#ifdef __cplusplus
#error The vl_atomic header does not work under C++ compilation due to the use of the _Atomic keyword.
#endif
...
/**
 * This header provides a wrapper over the C11 atomic functions, providing an
 * abstraction using the VL number types. Primarily used for "all in one place"
 * documentation, the header is intended to "smooth out" usage of atomic types
...
/**
 * \brief Memory order enumeration. Dictates reordering of memory accesses
 * surrounding and including atomic operations.
 *
...
typedef enum VL_MEMORY_ORDER
{
     ...
     * thread can be reordered before the load, nor after the store. All writes in
     * other threads that release the same atomic variable are visible before the
     * modification and the modification is visible in other threads that acquire
     * the same atomic variable.
     *
     * Useful for read-modify-write operations, specifically in lock-free data
     * structures.
     */
     ...
     *
     * Useful for the maximum amount of safety, probably at the cost of
     * performance.
     */
 ...
 * \return The previous value held by the atomic object referred to by the
 * specified pointer.
 */
 ...
 *
 * This operation replaces the values at ptr with desired, and returns its the
 * previous value.
 *
...
/**
 * \brief Atomically compares and exchanges the memory at ptr. If *ptr ==
 * *expectedPtr, *ptr = desired.
 *
 ...
 * \param desired Value to copy to the memory pointed to by ptr.
 * \return a boolean indicating the result of the comparison *ptr ==
 * *expectedPtr
 */
#define vlAtomicCompareExchangeWeak(ptr, expectedPtr, desired) atomic_compare_exchange_weak(ptr, expectedPtr, desired)

/**
 * \brief Atomically compares the memory at ptr with. If *ptr == *expectedPtr,
 * *ptr = desired.
 *
 * If the comparison is true, this performs a read-modify-write operation.
 * Otherwise, this only performs a read operation.
 ...
 * The memory order used for these operations is VL_MEMORY_ORDER_SEQ_CST.
 *
 * Guarantees that if the comparison fails, it’s because the value has actually
 * changed. Will never fail spuriously.
 *
 * \sa VL_MEMORY_ORDER_SEQ_CST
 * \param ptr Pointer to the atomic object to perform the operation on.
 * \param expectedPtr Pointer to do a bitwise comparison with.
 ...
 * *expectedPtr
 */
#define vlAtomicCompareExchangeStrong(ptr, expectedPtr, desired)                                                       \
    atomic_compare_exchange_strong(ptr, expectedPtr, desired)
...
#define vlAtomicCompareExchangeWeakExplicit(ptr, expectedPtr, desired, trueOrder, falseOrder)                          \
    atomic_compare_exchange_weak_explicit(ptr, expectedPtr, desired, (memory_order)(trueOrder),                        \
                                          (memory_order)(falseOrder))
...
#define vlAtomicCompareExchangeStrongExplicit(ptr, expectedPtr, desired, trueOrder, falseOrder)                        \
    atomic_compare_exchange_strong_explicit(ptr, expectedPtr, desired, (memory_order)(trueOrder),                      \
                                            (memory_order)(falseOrder))

// Thread Fencing

/**
 * \brief Prepares memory synchronization of non-atomic and relaxed atomic
 * accesses.
 *
 * \sa VL_MEMORY_ORDER
 ...
 *
 * This is the only way to properly initialize dynamically-allocated atomic
 * objects. Atomic objects allocated on the stack and directly assigned do not
