 ...
 *
 * Using extra state, a tagged pointer helps to distinguish atomic changes to
 * memory to eliminate the ABA problem. The pointer tag is a counter that is
 * incremented with write operation. The tag occupies as many bytes as the
 * native system pointer type, resulting in an 128-bit atomic type. On many
 * conventional modern platforms (x86_64, ARM64, etc) this is a well-supported
...
#endif
typedef VL_ALIGN_HINT(16) struct vl_tagged_ptr
{
    vl_uintptr_t ptr;
    vl_ularge_t tag;
} vl_tagged_ptr;
...
/**
 * \brief Atomic pointers must be aligned by their size when dynamically
 * allocated.
 */
 ...
 *
 * This function prevents the ABA problem by incrementing the tag every time the
 * pointer is updated.
 *
 * \param atomicPtr The atomic tagged pointer to store into.
 * \param ptr       The raw pointer to store.
 *
 * \sa vl_tagged_ptr, vlAtomicPtrCompareExchangeStrong,
 ...
 *
 * Compares \p *atPtr with \p *expected and, if equal, replaces it with \p
 * newValue and an incremented tag. Ensures full memory ordering via explicit
 * order parameters.
 *
 * \param atPtr      Pointer to the atomic tagged pointer.
 * \param expected   Pointer to the expected tagged pointer (updated on
 * failure).
 ...
 */
static inline vl_bool_t vlAtomicPtrCompareExchangeStrongExplicit(vl_atomic_ptr* atPtr, vl_tagged_ptr* expected,
                                                                 const void* newValue, vl_memory_order_t trueOrder,
                                                                 vl_memory_order_t falseOrder)
{
    (void)trueOrder;
    (void)falseOrder;
 ...
 *
 * Similar to \ref vlAtomicPtrCompareExchangeStrongExplicit, but may fail
 * spuriously. Intended for use in retry loops.
 *
 * \param atPtr      Pointer to the atomic tagged pointer.
 * \param expected   Pointer to the expected tagged pointer (updated on
 * failure).
 ...
 */
static inline vl_bool_t vlAtomicPtrCompareExchangeWeakExplicit(vl_atomic_ptr* atPtr, vl_tagged_ptr* expected,
                                                               const void* newValue, vl_memory_order_t trueOrder,
                                                               vl_memory_order_t falseOrder)
{
    (void)trueOrder;
...
/**
 * \brief Convenience macro for strong atomic tagged pointer CAS with sequential
 * consistency.
 *
...
/**
 * \brief Convenience macro for weak atomic tagged pointer CAS with sequential
 * consistency.
 *
 * \param atPtr        Pointer to atomic tagged pointer.
 * \param atPtrExpected Expected pointer/tag (updated on failure).
